...
import {
  createZapImoveisDetalheRouter,
  createZapImoveisRouter,
} from './routes.js';

export async function runZapImoveis(
  uploadMutex: Mutex = new Mutex(),
): Promise<ExecucaoStats> {
  const maxListingPages = getMaxListingPagesPerCrawl(OrigemAnuncio.ZAP_IMOVEIS);
  ...
  for (const entry of entries) {
    ...
    const crawler = new PlaywrightCrawler({
      ...
      sessionPoolOptions: {
        persistStateKeyValueStoreId: `${OrigemAnuncio.ZAP_IMOVEIS}-sessions`,
      },
      errorHandler: (context) => backoffOnRateLimit(context),
      failedRequestHandler: (context, error) => {
        reportFailedRequest(OrigemAnuncio.ZAP_IMOVEIS, context, error);
      },
    });
    stats.push(
  ...
  if (linksUnicos.size > 0) {
    const detalheQueue = await openFreshRequestQueue(
      `${OrigemAnuncio.ZAP_IMOVEIS}-detalhe`,
    );
  ...
  try {
    await uploadMutex.runExclusive(async () => {
      ...
      log.info(
        `ZAP Imóveis: ${String(capturas.length)} captura(s) bruta(s) enviada(s) ao bucket e registrada(s) em capturas_brutas`,
      );
    });
  } catch (error) {
    log.warning(
      'ZAP Imóveis: captura bruta falhou, run principal não é afetada',
      { error },
    );
    Sentry.captureException(error, {
      tags: { fonte: OrigemAnuncio.ZAP_IMOVEIS, fase: 'captura-bruta' },
    });
  }
