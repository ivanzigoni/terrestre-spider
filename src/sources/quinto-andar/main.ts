...
import {
  createQuintoAndarDetalheRouter,
  createQuintoAndarRouter,
} from './routes.js';
...
export async function runQuintoAndar(
  ...
  const capturaDataset = await openFreshDataset<RawCaptureItem>(
    `${OrigemAnuncio.QUINTO_ANDAR}-raw`,
  );
  ...
  for (const entry of entries) {
    ...
    const crawler = new HttpCrawler({
      ...
      requestHandler: createQuintoAndarRouter(dataset, capturaDataset),
      requestQueue,
      ...
      sessionPoolOptions: {
        persistStateKeyValueStoreId: `${OrigemAnuncio.QUINTO_ANDAR}-sessions`,
      },
  ...
  if (linksUnicos.size > 0) {
    const detalheQueue = await openFreshRequestQueue(
      `${OrigemAnuncio.QUINTO_ANDAR}-detalhe`,
    );
    ...
    const detalheCrawler = new HttpCrawler({
      httpClient: new ImpitHttpClient({ browser: Browser.Chrome }),
      requestHandler: createQuintoAndarDetalheRouter(capturaDataset),
  ...
  try {
    ...
    log.warning(
      'Quinto Andar: captura bruta falhou, run principal não é afetada',
      { error },
    );
    Sentry.captureException(error, {
      tags: { fonte: OrigemAnuncio.QUINTO_ANDAR, fase: 'captura-bruta' },
    });
  }
