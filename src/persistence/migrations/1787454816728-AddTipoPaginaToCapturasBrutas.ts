...
export class AddTipoPaginaToCapturasBrutas1787454816728 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `CREATE TYPE "${schema}"."capturas_brutas_page_type_enum" AS ENUM('listagem', 'detalhe')`,
    );
    await queryRunner.query(
      `ALTER TABLE "capturas_brutas" ADD "page_type" "${schema}"."capturas_brutas_page_type_enum" NOT NULL DEFAULT 'listagem'`,
    );
  }
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `DROP TYPE "${schema}"."capturas_brutas_page_type_enum"`,
    );
  }
}
