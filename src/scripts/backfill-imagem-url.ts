...
/**
 * Uso: node --env-file-if-exists=.env --import tsx src/scripts/backfill-imagem-url.ts
...
async function buscarLote(
  ...
  return avistamentoRepo
    ...
    .where('avistamento.imagemUrl IS NULL')
...
async function processarLinha(
  ...
  try {
    ...
    await avistamentoRepo.update(linha.id, {
      imagemUrl: resultado.data.imagemUrl,
    });
    return 'atualizada';
  } catch (error) {
    const mensagem = error instanceof Error ? error.message : String(error);
    log.warning(
      `backfill-imagem-url: avistamento #${String(linha.id)} (${linha.anuncio.origem}) falhou: ${mensagem}`,
    );
    return 'erro';
  }
}
...
async function main(): Promise<void> {
  ...
  try {
    ...
    log.info(
      `backfill-imagem-url: ${String(totalPendente)} avistamento(s) pendente(s)${sufixoLimite}`,
    );
    ...
    for (;;) {
      ...
      log.info(
        `backfill-imagem-url: ${String(total.atualizadas)} atualizada(s), ${String(total.comErro)} com erro até agora (último id ${String(ultimoId)})`,
      );
    }

    log.info(
      `backfill-imagem-url: concluído — ${String(total.atualizadas)} atualizada(s), ${String(total.comErro)} com erro`,
    );
  } finally {
